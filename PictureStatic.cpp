...
void CPictureStatic::SetImagePath(CString strPath)
{
	...
	if(!m_pBackSkin->LoadPath(strPath))
	{
		delete m_pBackSkin;
		m_pBackSkin = NULL;
		return;
...
void CPictureStatic::SetImage(LPCTSTR pName, LPCTSTR pType, HMODULE hInst)
{
	...
	if (!m_pBackSkin->Load(pName, pType, hInst))
	{
		delete m_pBackSkin;
		m_pBackSkin = NULL;
		return;
...
{		
	...
	if(m_pBackSkin)
	{
		Unit units;
		RectF srcRect;
		m_pBackSkin->m_pBitmap->GetBounds(&srcRect, &units);
		graphics.DrawImage(m_pBackSkin->m_pBitmap, destRect, srcRect.X, srcRect.Y, srcRect.Width, srcRect.Height, UnitPixel, NULL);
	}
}
...
void CPictureStatic::OnDrawLayerdWindow(Graphics& Gps)
{
	...
	{		
		...
		RectF srcRect;
		m_pBackSkin->m_pBitmap->GetBounds(&srcRect, &units);
		Gps.DrawImage(m_pBackSkin->m_pBitmap, destRect, srcRect.X, srcRect.Y, srcRect.Width, srcRect.Height, UnitPixel, NULL);
	}
}
